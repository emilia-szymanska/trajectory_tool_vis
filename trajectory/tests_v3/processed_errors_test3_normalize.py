...
def plot_6boxplots(data_x_unsmoothed, data_y_unsmoothed, data_euclid_unsmoothed, data_x_smoothed, data_y_smoothed, data_euclid_smoothed):
    ...
    ax.boxplot(box_data, labels=labels, patch_artist=True, boxprops=boxprops, flierprops=flierprops, medianprops=medianprops)
...
def plot_histograms(data_unsmoothed, data_smoothed, bins=20):
    # Determine the data range for the bins
    combined_data = np.concatenate((data_smoothed, data_unsmoothed))
    data_range = (np.min(combined_data), np.max(combined_data))
    
    # Plot the histogram, with density=True to normalize the area to 1
    plt.hist(data_unsmoothed, bins=bins, label="Unsmoothed trajectory", range=data_range, color="g", alpha=0.7, density=True)
    plt.hist(data_smoothed, bins=bins, label="Smoothed trajectory", alpha=0.5, range=data_range, color="b", density=True)
...
input_file_smoothed = "../../data/tests_v3/errors_smoothed_all.csv"
input_file_unsmoothed = "../../data/tests_v3/errors_unsmoothed_all.csv"

input_file_x_smoothed = "../../data/tests_v3/errors_x_smoothed.csv"
input_file_x_unsmoothed = "../../data/tests_v3/errors_x_unsmoothed.csv"

input_file_y_smoothed = "../../data/tests_v3/errors_y_smoothed.csv"
input_file_y_unsmoothed = "../../data/tests_v3/errors_y_unsmoothed.csv"
...
error_unsmoothed = random_discard(error_unsmoothed, target_length=len(error_smoothed))

print(len(error_smoothed))
print(len(error_unsmoothed))
# error_smoothed = min_max_normalize(error_smoothed)
# error_unsmoothed = min_max_normalize(error_unsmoothed)
print("=====")
