...
csvfile = open("../data/smoothed3_gtsam_pose.csv")
...
i = 40
p0 = (rows[i]['pos_x'], rows[i]['pos_y'], 0.0)
# q0 = (rows[i]['qx'], rows[i]['qy'], rows[i]['qz'], rows[i]['qw'])
q0 = (0.0, 0.0, -0.635, 1)


csvfile = open("../data/smoothed31_gtsam_pose.csv")
csvreader = csv.reader(csvfile, delimiter='\t')
header = next(csvreader)
