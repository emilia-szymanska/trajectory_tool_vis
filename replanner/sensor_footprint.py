...
for i, pose in enumerate(global_poses.poses):
    ...
    # ax.fill(robot[:,0], robot[:,1], alpha=0.5, fc='c', ec='none')
    polygon = Polygon(robot,  alpha=0.5, fc='c', ec='none')
...
for i, pose in enumerate(global_poses.poses):
    if i < 400:
        ...
        arrow = FancyArrow(position[0], position[1], orientation[0], orientation[1], color='k')
        ax.add_patch(arrow)
    else:
...
plt.title("Sensor footprint's hull")
plt.xlabel('X')
plt.ylabel('Y')
plt.grid(True)
plt.show()